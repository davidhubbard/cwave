...
/* get winsock running */
    ...
    while(!sm_shutdown) {
        mio_run(sm->mio, -1); /* -1 = wait indefinitely - SIGINT and others will break out of this */

        if(sm_logrotate) { /* received SIGHUP */
        ...
        if(sm_lost_router) { /* event_CLOSE, which will break out of mio_run() */
            if(sm->retry_left < 0) {
