    ...
    switch(e) {
        ...
        case event_OPEN:
            ...
            do {
                char *domain;
                int domain_len;
                xhash_iter_get(sm->hosts, (void *) &domain, &domain_len, NULL);

                /* skip already requested SM id */
                if (strlen(sm->id) == domain_len && strncmp(sm->id, domain, domain_len) == 0)
                    continue;

                nad = nad_new();
                ns = nad_add_namespace(nad, uri_COMPONENT, NULL);
                nad_append_elem(nad, ns, "bind", 0);
                nad_set_attr(nad, nad->ecur - 1, -1, "name", domain, domain_len);
                nad_append_attr(nad, -1, "multi", "to");
                log_debug(ZONE, "requesting domain bind for '%.*s'", domain_len, domain);
                sx_nad_write(sm->router, nad);
